...
from ttsreal import EdgeTTS,SovitsTTS,XTTS,CosyVoiceTTS,FishTTS,TencentTTS,DoubaoTTS,DoubaoTTSV3
...
def read_imgs(img_list):
    ...
    for img_path in tqdm(img_list):
        ...
        if frame is None:
            logger.error(f"[read_imgs] Failed to read image: {img_path}")
        else:
            logger.info(f"[read_imgs] Successfully read image: {img_path}, shape: {frame.shape}")
            frames.append(frame)
    return frames
...
class BaseReal:
    def __init__(self, opt):
        ...
        elif opt.tts == "doubaov3":
            self.tts = DoubaoTTSV3(opt,self)
    ...
    def process_frames(self,quit_event,loop=None,audio_track=None,video_track=None):
        ...
        while not quit_event.is_set():
            ...
            if self.opt.transport=='virtualcam':
                ...
                image = combine_frame
                if image is None:
                    logger.error("[process_frames] image is None before VideoFrame.from_ndarray, idx=%s", idx)
                else:
